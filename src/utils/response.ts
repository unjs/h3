...
import { sanetizeStatusCode, sanetizeStatusMessage } from "./sanetize";
...
export function sendNoContent(event: H3Event, code = 204) {
  event.node.res.statusCode = sanetizeStatusCode(code, 204);
...
export function setResponseStatus(
  ...
  if (code) {
    event.node.res.statusCode = sanetizeStatusCode(
      code,
      event.node.res.statusCode
    );
  }
  if (text) {
    event.node.res.statusMessage = sanetizeStatusMessage(text);
  }
}
