...
describe("app", () => {
  ...
  it("calls onRequest and onResponse when an error is returned", async () => {
    ...
    // expect(ctx.onError).toHaveBeenCalledTimes(1); // TODO
  ...
  it.todo(
    "calls onRequest and onResponse when an unhandled error occurs",
    async () => {
      ctx.app.use(
        eventHandler((event) => {
          // @ts-expect-error
      ...
      // expect(onRequest).toHaveBeenCalledTimes(1); // TODO
      // expect(ctx.onRequest.mock.calls[0][0].path).toBe("/foo");
