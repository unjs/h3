...
describe("", () => {
  ...
  describe('serializeIterableValue', () => {
    ...
    it.each([
      ...
      { value: Buffer.from("Hello, world!"), output: Buffer.from("Hello, world!") },
  ...
  describe("sendIterable", () => {
    it("sends empty body for an empty iterator", async () => {
      app.use(
        eventHandler((event) => sendIterable(event, []))
      );
      const result = await request.get('/');
    ...
    describe('iterable support', () => {
      it.each([
        ...
        { type: 'Map.keys()', iterable: new Map([['the-value', 'unused']]).keys() },
        { type: 'Map.values()', iterable: new Map([['unused', 'the-value']]).values() },
        { type: 'Iterator object', iterable: { next: () => ({ value: 'the-value', done: true }) } },
        { type: 'AsyncIterator object', iterable: { next: () => Promise.resolve({ value: 'the-value', done: true }) } },
        { type: 'Generator (yield)', iterable: (function*(){ yield 'the-value'; })() },
        { type: 'Generator (return)', iterable: (function*(){ return 'the-value'; })() },
        {
          type: 'Generator (yield*)', iterable: (function*() {
            yield * ['the-value'];
          })(),
        },
        {
          type: 'AsyncGenerator', iterable: (async function*() {
            await Promise.resolve();
            yield 'the-value';
          })(),
        },
        {
          type: 'ReadableStream (push-mode)', iterable: new ReadableStream({
            start(controller) {
        ...
        {
          type: 'ReadableStream (pull-mode)', iterable: new ReadableStream({
            pull(controller) {
              controller.enqueue('the-value');
              controller.close();
            },
    ...
    describe('serializer argument', () => {
      it('is called for every value', async () => {
        ...
        app.use(eventHandler(event => sendIterable(event, iterable, serializer)));
        const response = await request.get('/');
